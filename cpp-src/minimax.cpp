...
Move Minimax::findBest(char board[8][8], bool turn){
	...
	for (Move& mv : all_moves){
		...
		float val = abMinimax(board, depth-1, !turn, alpha, beta);
...
int Minimax::abMinimax(char board[8][8], int depth, bool turn, float alpha, float beta){
	if (depth == 0){
		return abMinimaxAttacks(board, !turn, -INFINITY, INFINITY);
	}else{
		float best_val = (turn) ? -INFINITY : INFINITY;
...
float Minimax::abMinimaxAttacks(char board[8][8], bool turn, float alpha, float beta){
	...
	if (attacks.empty()){
		...
		for (Move& mv : attacks){
			...
			float val = abMinimaxAttacks(board, !turn, alpha, beta);
...
// This function checks if a generated move is valid, and if so it adds the move to `moves`
bool Minimax::tryAddMove(const char board[8][8], const Move &mv, const bool &turn, vector<Move> moves){
...
// This function checks if a generated attack is valid, and if so it adds the move to `attacks`
...
void Minimax::generatePawnMove(const char board[8][8], const Square &sq, const bool turn, vector<Move> &moves){
	...
	char promotions[4] = (turn) ? {'Q','N','R','B'} : {'q','n','r','b'};
	//Move 1 ahead
	target_sq.y += dir;
...
void Minimax::generatePawnAttack(const char board[8][8], const Square &sq, const bool turn, vector<Move> &attacks){
	Square target_sq(sq); //Temporary target square for move
	int dir = (turn) ? 1 : -1; //Direction pawn is moving (white moves +, black moves -)
	char promotions[4] = (turn) ? {'Q','N','R','B'} : {'q','n','r','b'};
...
void Minimax::generateKnightAttack(const char board[8][8], const Square &sq, const bool turn, vector<Move> &attacks){
	Square targets[8] = {Square(sq.x - 2, sq.y + 1), Square(sq.x - 2, sq.y - 1), Square(sq.x - 1, sq.y + 2), Square(sq.x - 1, sq.y - 2), Square(sq.x + 2, sq.y + 1), Square(sq.x + 2, sq.y - 1), Square(sq.x + 1, sq.y + 2), Square(sq.x + 1, sq.y - 2)}
	for (int i=0;i<8;i++){
...
void Minimax::generateBishopMove(const char board[8][8], const Square &sq, const bool turn, vector<Move> &moves){
	...
	for (int i=0;i<4;i++){
		target = sq + dirs[i];
		while (true){
			if(!tryAddMove(board, Move(sq, targets[i]), turn, moves)){
				break;
			}
			target += dirs[i]
		}
...
void Minimax::generateBishopAttack(const char board[8][8], const Square &sq, const bool turn, vector<Move> &attacks){
	Square dirs[4] = {Square(1, 1), Square(-1, 1), Square(1, -1), Square(-1,-1)};
	Square target;
	for (int i=0;i<4;i++){
		target = sq + dirs[i];
...
void Minimax::generateRookMove(const char board[8][8], const Square &sq, const bool turn, vector<Move> &moves){
...
void Minimax::generateRookAttack(const char board[8][8], const Square &sq, const bool turn, vector<Move> &attacks){
	Square dirs[4] = {Square(1, 0), Square(-1, 0), Square(0, -1), Square(0, 1)};
	Square target;
	for (int i=0;i<4;i++){
		target = sq + dirs[i];
...
void Minimax::generateQueenMove(const char board[8][8], const Square &sq, const bool turn, vector<Move> &moves, vector<Move> &attacks){
...
void Minimax::generateKingMove(const char board[8][8], const Square &sq, const bool turn, vector<Move> &moves){
...
void Minimax::generateKingAttack(const char board[8][8], const Square &sq, const bool turn, vector<Move> &attacks){
	Square targets[8] = {Square(sq.x - 1, sq.y - 1), Square(sq.x - 1, sq.y), Square(sq.x - 1, sq.y + 1), Square(sq.x, sq.y - 1), Square(sq.x, sq.y + 1), Square(sq.x + 1, sq.y - 1), Square(sq.x + 1, sq.y), Square(sq.x + 1, sq.y + 1)}
	for (int i=0;i<8;i++){
		tryAddAttack(board, Move(sq, targets[i]), turn, attacks);
	}
...
Square Minimax::findKing(const board[8][8], const bool &turn){
	char king = (turn) ? 'K' : 'k';
	for (int i=0;i<8;i++){
		for (int j=0;j<8;j++){
			if (board[i][j]==king){
